...
def visualizacao_maxmin_album(df, coluna):
    ...
    try:
        ...
        plt.savefig ("./Imgs/graficoalbum", dpi=600, bbox_inches='tight')
        plt.close()
    except KeyError:
       print(f'A entrada {coluna} não é uma coluna válida do df. Tente uma coluna existente.')
...
def visualizacao_maxmin_musicas(df, coluna):
    ...
    try:
        ...
        fig = sns.barplot(data = principal, x = coluna, y = "nome") #Criando gráfico em barras
        plt.savefig ("./Imgs/graficodiscografia", dpi=600, bbox_inches='tight')
        plt.close()
    except KeyError:
       print(f'A entrada {coluna} não é uma coluna válida do df. Tente uma coluna existente.')
...
def corr_com_tempo(df, variavel):
    ...
    print(f"A correlação entre a duração da musica e {variavel} é: {np.corrcoef(duracao, var)[0][1]  }")
...
def plot(data, coluna1, coluna2 ):
    ...
    """
    variaveis_aceitas = ['popularidade','volume', 'bpm', 'energia', 'fala', 'acustica', 'instrumentalidade', 'valencia', 'vivacidade', 'dancabilidade']
    ...
    try:
        ...
        ax.legend()
        plt.savefig (f"./Imgs/{coluna1} x {coluna2}", dpi=600, bbox_inches='tight')
        plt.close()
    except Exception as error:
